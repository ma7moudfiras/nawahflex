...
### صفر نطاقات خارجية
الموقع **لا يطلب شيئاً من أي نطاق غير نطاقه** (عدا Supabase عند إرسال نموذج).
الخطوط مستضافة محلياً في `site/assets/fonts/` — لا `fonts.googleapis.com`
ولا `fonts.gstatic.com`. السبب: كل نطاق خارجي في الرأس يضيف بحث DNS ومصافحة
TLS قبل أول رسم، ويجعل شكل الموقع رهيناً بوصول جهة ثالثة.

**تحديث الخطوط** (عند تغيير وزن أو إضافة عائلة):
1. عدّل قائمة الأوزان في الرابط، واجلب الورقة بترويسة متصفح حديث:
   `curl -A "Mozilla/5.0 ... Chrome/120" "https://fonts.googleapis.com/css2?family=..."`
2. أبقِ المجموعتين `arabic` و `latin` فقط — احذف البقية.
3. نزّل ملفات `.woff2` إلى `site/assets/fonts/` وأعد توليد `site/css/fonts.css`.
